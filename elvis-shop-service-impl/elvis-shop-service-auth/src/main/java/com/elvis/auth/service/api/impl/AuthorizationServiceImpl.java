package com.elvis.auth.service.api.impl;
...
import com.elvis.auth.mapper.AppInfoMapper;
import com.elvis.auth.mapper.entity.ElvisAppInfo;
import com.elvis.auth.utils.Guid;
...
public class AuthorizationServiceImpl extends BaseApiService<JSONObject> implements AuthorizationService {
	...
	public BaseResponse<JSONObject> applyAppInfo(String appName) {
		...
		// 3.添加数据库中
		ElvisAppInfo elvisAppInfo = new ElvisAppInfo(appName, appId, appScrect);
		int insertAppInfo = appInfoMapper.insertAppInfo(elvisAppInfo);
	...
	public BaseResponse<JSONObject> getAccessToken(String appId, String appSecret) {
		...
		ElvisAppInfo elvisAppInfo = appInfoMapper.selectByAppInfo(appId, appSecret);
		if (elvisAppInfo == null) {
			return setResultError("appId或者是appSecret错误");
		}
		// 3.获取应用机构信息 生成accessToken
		String dbAppId = elvisAppInfo.getAppId();
	...
	public BaseResponse<JSONObject> getAppInfo(String accessToken) {
		...
		// 3.使用appid查询数据库
		ElvisAppInfo elvisAppInfo = appInfoMapper.findByAppInfo(appId);
		if (elvisAppInfo == null) {
			return setResultError("AccessToken  invalid");
		}
		// 4.返回应用机构信息
		JSONObject data = new JSONObject();
		data.put("appInfo", elvisAppInfo);
		return setResultSuccess(data);
	}
